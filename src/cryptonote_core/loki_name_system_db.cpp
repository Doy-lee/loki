      ...
      case SQLITE_ROW:
      {
        switch (type)
        {
          ...
          case lns_sql_type::get_mapping:
          {
            ...
            if (validate_lns_name_value_mapping_lengths(nettype, entry->type, name, name_len, value, value_len))
            {
...
bool validate_lns_name_value_mapping_lengths(cryptonote::network_type nettype, uint16_t type, char const *name, int name_len, char const *value, int value_len)
{
...
{
  if (!validate_lns_name_value_mapping_lengths(nettype, entry.type, entry.name.data(), static_cast<int>(entry.name.size()), entry.value.data(), static_cast<int>(entry.value.size())))
  {
